// $Id: inc.js.restrict-width.js 715 2011-06-12 20:06:27Z conor $
...
function CeonRestrictWidth(min_width_val, max_width_val)
{
	...
	if (screen.width > max_width) {
		...
		if (!restrict_width_css_rule_obj) {
			// Rule must be added as could not identify one (either because none exists or because
			// of a browser security restriction, e.g. chrome with the file:// protocol)
				...
				if (document.styleSheets[0].cssRules != undefined) {
					...
					try {
						var inserted_at = document.styleSheets[0].insertRule(
							'@media screen { ' + selector + '{position:absolute; width:' +
							_determineHSize() + '; left:' + _determineHPos() + ';}' + ' }',
							last_rule);
						
						restrict_width_css_rule_obj =
							document.styleSheets[0].cssRules[inserted_at].cssRules[0];
						...
						document.styleSheets[last_style_sheet].insertRule(
							selector + ' {position:absolute; width:' + _determineHSize() +
								'; left:' + _determineHPos() + ';}', 0); 
						
						restrict_width_css_rule_obj =
							document.styleSheets[last_style_sheet].rules[0];
					}
				} else {
					CeonAddStyleSheet('screen');
	 ...
	 * @member  CeonRestrictWidth
	 * @private
	 * @returns {string}   The string to which the left position of the resizeable div should be
	 *                     set.
	 */
