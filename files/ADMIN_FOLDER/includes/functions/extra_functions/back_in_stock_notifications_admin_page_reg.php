 ...
 * @version     $Id: back_in_stock_notifications_admin_page_reg.php 2023-11-20 torvista
 */

if (!defined('IS_ADMIN_FLAG')) {
    die('Illegal Access');
}
// if a cron script calls application_top, this function may not be available at this point
if (function_exists('zen_page_key_exists')) {
    if (!zen_page_key_exists('ceon_bisn')) {
        ...
        if (sizeof($error_messages) > 0) {
            ...
            // note language definitions are not available at this stage, so need to use hard-coded texts
            $messageStack->add('BISN Installer: menu item added "Catalog->Ceon Back In Stock Notifications"', 'success');
        }
    }
}
